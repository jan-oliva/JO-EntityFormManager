...
class EntityFormManager
{
     ...
     */
    public function __construct(Form $form, $entity, EntityManager $em, $formFieldPrefix = self::FORM_FIELD_PREFIX)
    {
        $this->form = $form;
     ...
     * @param bool $floatHtmlInputTypeIsNumber
     * @param bool $intHtmlInputTypeIsNumber
     */
    public function createFields($items = array(), $itemsIsWhitelist = false, \Nette\Forms\Container $container = null, $autoRequire = false, $floatHtmlInputTypeIsNumber = true, $intHtmlInputTypeIsNumber = true)
    {
        ...
        foreach ($fields as $prop) {
            ...
            switch ($fieldType) {
                ...
                case 'bigint':
                    $el = $object->addText($fieldName, $caption);
                    if ($intHtmlInputTypeIsNumber) {
                        $el->setHtmlAttribute('type', 'number');
                    }
                    $this->makeElementRequired($el, $prop, $autoRequire, $caption);
                    break;
                case 'text':
